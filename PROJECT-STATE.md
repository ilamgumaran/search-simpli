...
- The dependency-free Python suite now executes 67/67 tests; the latest recorded
  pinned-toolchain Zig run executes 77/77 tests (57 before round A, 59 with the
  C ABI, 77 with the native chunker/analyzer-v2/CLI), plus `zig build test-abi`'s
  12/12 C-ABI conformance checks.
