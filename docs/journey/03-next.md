...
1. **`simpli-helper` M12-T0 adopts the core.** The app replaces its Dart lexical port with this package: add `bindings/dart/search_simpli` as a path dependency, package `libsearch_simpli.so` as a jniLib for `arm64-v8a`, pin `CONTRACTS_VERSION`, and delete the duplicate ranker. Everything it needs exists and is proven on-device — `ss_open`/`ss_query`/`ss_evidence`/`ss_status` and, since round B, publishing into the app's own private storage. Two things to fix or work around on the way: the macOS loader is working-directory-relative (the app is Android, so this only bites development), and the package does not bind `ss_index_folder` yet.
2. **`simpli-helper` M11-T1 uses `ss_index_folder`.** Once the app is on the core, indexing a real folder on the device is a call, not a port: `ss_index_folder(dir, folder, {"update": true})` on a schedule or on demand, reading the report to decide what to tell the family. This needs a Dart binding for that one function first (S1-T2 deliberately did not add one while S1-T3 was in flight), which is a small follow-up task in this repo.
